...
cd $disttop/glib-2
unzip -q -o $disttop/3rdparty/glib_2*win32.zip
unzip -q -o $disttop/3rdparty/glib-dev_2*win32.zip

cd $top
make clean
GLIB2_LIBS="-L$disttop/glib-2/lib -lglib-2.0 -lgobject-2.0" GLIB2_CFLAGS="-I$disttop/glib-2/include/glib-2.0 -I$disttop/glib-2/lib/glib-2.0/include" CC="$win32gcc -m32 -static" ./configure --host x86_64-w64-mingw32 --without-magick --without-pstoedit
make

cd $disttop
makensis -DVERSION=$APP_VERSION-$(date +%Y%m%d) $APP_NAME.nsi
mkdir -p ../out
zip=../out/$APP_NAME-$APP_VERSION-$(date +%Y%m%d)-win32-setup.zip
rm -f  $zip
